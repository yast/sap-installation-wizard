...
hana_lcm_workflow()
{
   ...
   if [ -d ${MEDIA_TARGET}/Instmaster/DATA_UNITS/HDB_SERVER_LINUX_${ARCH} ]; then
       ...
       if [ "${ARCH}" == "X86_64" ];then
           ...
           if [ "${ARCH}" == "PPC64" ];then
              ...
              echo "Cannot install HANA. Platform ${ARCH} not supported by SUSE installation wizard."
              rc=1
           fi
       fi
   ...
   if [ ! -z "${missing}" ]; then
       echo -e "Cannot install HANA. The following folders are expected on the media:\n${missing}"
       rc=1
   else
...
###########################################
   ...
   if [ -n "${HDBLCM}" ]; then
      ...
      missing=$(hana_check_components)
      if [ ! -z ${missing} ]; then
         echo "Cannot install, HANA component folders missing on media: ${missing}"
         rc=1
      else
         hana_unified_installer_workflow
         rc=$?
         ...
         if [ -f ${MEDIA_TARGET}/Instmaster/DATA_UNITS/HDB_CLIENT_LINUXINTEL/hdbinst ]; then
            ...
            if [ -f ${MEDIA_TARGET}/Instmaster/DATA_UNITS/SAP_HANA_CLIENT32/SAP_HANA_CLIENT/hdbinst ]; then
               ...
               if [ -f ${MEDIA_TARGET}/Instmaster/DATA_UNITS/SAP\ HANA\ CLIENT\ 1.0\ FOR\ B1/LINX32SUSE/SAP_HANA_CLIENT/hdbinst ]; then
                  ...
                  echo "Cannot find HANA 32-bit client, please install manually before SAP BusinessOne installation"
               fi
            fi
         fi
      fi
